...
# MAX_NONCE = (2 ** (2 ** (2 ** 3))) - 1  # 16^64 - 1 = 2^256 - 1, the maximum value of the nonce
#
# class Block:
#     def __init__(self, index: int, timestamp: float, data: str, previous_hash: str = '') -> None:
#         self.index = index
...
#         return sha.hexdigest()
#
#     def mine(self, difficulty: int, base: int = 16) -> None: # todo any disadvantages of using base 2?
...
#
#             # Convert the hash to the chosen numeral system (base = 2, 8, 16) # todo add 4, 10, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536, 131072, 262144, 524288, 1048576
#             if base == 2:
#                 # Convert hash to binary
#                 converted_hash = bin(int(self.hash, 16))[2:].zfill(256)
#             elif base == 8:
#                 # Convert hash to octal
#                 converted_hash = oct(int(self.hash, 16))[2:].zfill(85)  # ~85 characters
#             elif base == 16:
...
class Block:
    ...
    def mine(self, difficulty: int, base: int = 2) -> None:
        ...
        while True:
            ...
            # Convert the hash to the chosen numeral system (base = 2, 8, 16)
            if base == 2:
                # Convert hash to binary
                converted_hash = bin(int(self.hash, 16))[2:].zfill(256)
            elif base == 8:
...
#         self.target_block_time = target_block_time  # Target block time in seconds
#
#     def add_block(self, new_block: Block, difficulty_coefficient: float, base: int = 16) -> None:
#         new_block.previous_hash = self.get_latest_block().hash  # Set the previous hash of the new block to the hash of the latest block
#         new_block.mine(self.difficulty, base)  # Pass the chosen numeral system
#         self.chain.append(new_block)
#         self.adjust_difficulty(difficulty_coefficient)
#         log_validity(self)
#         logger.debug(f"Difficulty: {self.difficulty}")
...
if __name__ == "__main__":
    ...
    mine_blocks(
        blockchain,
        num_blocks=10,
        difficulty_coefficient=1.5, # todo why 1.5 not 2?
    )
