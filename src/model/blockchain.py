...
from src.constants import HASH_BIT_LENGTH, BASE, DEFAULT_PRECISION, AVERAGE_MINING_TIME_ADJUSTMENT_INTERVAL_KEY, \
    REVERSED_ADJUSTMENT_FACTOR_KEY, INITIAL_BIT_DIFFICULTY, TARGET_BLOCK_MINING_TIME, ADJUSTMENT_BLOCK_INTERVAL, \
...
# src/model/blockchain.py

class Blockchain:
    def __init__(self, number_blocks_to_add: int, number_blocks_slice: int) -> None:
        self.logger = configure_logging()
        ...
        self.bit_difficulties = [self.proof_of_work.initial_bit_difficulty]
    ...
    def add_block(self, new_block: Block) -> None:
        new_block.previous_hash = self.get_latest_block().hash if self.blocks else GENESIS_BLOCK_PREVIOUS_HASH
        self.proof_of_work.find_nonce(new_block)
        new_block.timestamp = time.time()
        actual_mining_time = new_block.timestamp - self.get_latest_block().timestamp

        if not self.proof_of_work.validate_proof(new_block):
            self.logger.error(f"Block {new_block.index} was mined with a hash that does not meet the difficulty")
            self.logger.error(f"Block hash: {new_block.hash}")
            self.logger.error(f"Target value: {(BASE ** (HASH_BIT_LENGTH - self.bit_difficulties[-1])) - 1}")
            return
    ...
    def adjust_difficulty(self) -> None:
        if (len(self.blocks) - 1) % self.proof_of_work.adjustment_block_interval == 0:
            average_mining_time_adjustment_interval = self.get_average_mining_time(self.proof_of_work.adjustment_block_interval)
            reversed_adjustment_factor = average_mining_time_adjustment_interval / self.proof_of_work.target_block_mining_time

            self.logger.debug(f"{AVERAGE_MINING_TIME_ADJUSTMENT_INTERVAL_KEY}: {average_mining_time_adjustment_interval:.{DEFAULT_PRECISION}f} seconds")
            ...
            if reversed_adjustment_factor > 0:
                bit_adjustment_factor = math.log2(reversed_adjustment_factor)
                clamped_bit_adjustment_factor = self.proof_of_work.clamp(bit_adjustment_factor)
                new_bit_difficulty = max(self.proof_of_work.smallest_bit_difficulty, last_bit_difficulty - clamped_bit_adjustment_factor)
            else:
                new_bit_difficulty = self.proof_of_work.smallest_bit_difficulty
