...
MAX_NONCE_INT = 2 ** NONCE_BIT_LENGTH - 1  # The maximum value of the nonce
...
class Block:
    ...
    def mine(
        ...
        max_target_hash_int: float = math.pow(2, HASH_BIT_LENGTH - bit_difficulty) - 1  # todo move to blockchain.py
        ...
        while True:
            ...
            hash_int: int = int(self.hash, 16)

            # Check if the hash value is less than the target value: if so, the block is mined
            if hash_int < max_target_hash_int:  # If the hash meets the target value, the block is mined
                break
