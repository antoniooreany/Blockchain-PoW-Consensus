...
def find_nonce(block, bit_difficulty: float) -> int:
    """
    ...
    log_mined_block(block)  # todo uncomment this line after implementing the logging

    return block.nonce


def validate_proof(block, bit_difficulty: float) -> bool:
    """
...
def clamp_bit_adjustment_factor(bit_adjustment_factor: float, bit_clamp_factor: float) -> float:
    """
